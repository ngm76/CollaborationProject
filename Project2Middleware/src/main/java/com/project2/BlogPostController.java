...
import com.project2.dao.BlogPostDao;
...
public class BlogPostController {
	...
	public ResponseEntity<?> addBlogPost(@RequestBody BlogPost blogPost, HttpSession session) {
		...
		try {
			...
			ErrorClass errorClass = new ErrorClass(6, "Unable to post blogpost details.." + e.getMessage());
	...
	public ResponseEntity<?> blogsWaitingForApproval(HttpSession session) {
		...
		// CHECK FOR AUTHORIZATION - Only admin can view list of blogs waiting for
		// approval
	...
	public ResponseEntity<?> approveBlogPost(@RequestBody BlogPost blogPost, HttpSession session) {
		...
		try {
			...
			ErrorClass errorClass = new ErrorClass(7, "Unable to approve the blogpost " + e.getMessage());
			return new ResponseEntity<ErrorClass>(errorClass, HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}

	@RequestMapping(value = "/reject/{rejectionReason}", method = RequestMethod.PUT)
	public ResponseEntity<?> rejectBlogPost(@PathVariable String rejectionReason, @RequestBody BlogPost blogPost,
			HttpSession session) {
		String email = (String) session.getAttribute("email");
		...
		try {
			...
			ErrorClass errorClass = new ErrorClass(7, "Unable to delete the blogpost " + e.getMessage());
			return new ResponseEntity<ErrorClass>(errorClass, HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
}